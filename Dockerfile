...
# and `pip install mi_agent` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "mi_agent", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n mi_agent -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "mi_agent", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "mi_agent", "YOUR_CONSOLE_SCRIPT"]
